...
The `bytearray` generate a binary with data between 0x01 and 0xff. In general the created file is
used to compare (using the `bincompare` command) with a memory data in order to check badchars.
